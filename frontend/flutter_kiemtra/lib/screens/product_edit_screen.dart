...
class _ProductEditScreen extends State<ProductEditScreen> {
  ...
  Book _editedProduct = Book(id: 0, name: '',author: '', description: '', unitPrice: 0, imageUrl: '');
  bool _isInit = true;
  ...
  void didChangeDependencies() {
    if (_isInit) {
      if (ModalRoute.of(context)!.settings.arguments != null) {
        ...
        if (productId != 0) {
          _editedProduct = Provider.of<BooksProvider>(context, listen: false).findById(productId);
  ...
  Future<void> _saveForm() async {
    ...
    if (_editedProduct.id !=0) {
      ...
      try {
          await Provider.of<BooksProvider>(context, listen: false).updateProduct(_editedProduct.id, _editedProduct);
      ...
      try {
          await Provider.of<BooksProvider>(context, listen: false).addProduct(_editedProduct);
          Navigator.of(context).pop();
          setState(() {_isLoading = false;});
      } catch (error) {
        await showDialog(context: context, builder: (ctx) => AlertDialog(
        ...
        Padding(
          ...
          child: Form(key: _form, child: SingleChildScrollView(child: Column(children: [
            TextFormField(decoration: InputDecoration(labelText: 'Title'), textInputAction: TextInputAction.next,
              ...
              onSaved: (value) => _editedProduct = Book(name: value!,author:_editedProduct.author, description: _editedProduct.description, unitPrice: _editedProduct.unitPrice, imageUrl: _editedProduct.imageUrl, id: _editedProduct.id),
              validator: (value) {
            ...
            TextFormField(decoration: InputDecoration(labelText: 'Price'), focusNode: _priceFocusNode, textInputAction: TextInputAction.next, keyboardType: TextInputType.numberWithOptions(signed: true, decimal: true),
              ...
              onSaved: (value) => _editedProduct = Book(name: _editedProduct.name,author: _editedProduct.author, description: _editedProduct.description, unitPrice: double.parse(value!), imageUrl: _editedProduct.imageUrl, id: _editedProduct.id),
              validator: (value) {
                if(value!.isEmpty) { return 'Please provide a value'; }
            ...
            TextFormField(decoration: InputDecoration(labelText: 'Description'), focusNode: _descriptionFocusNode, maxLines: 3, textInputAction: TextInputAction.next, keyboardType: TextInputType.multiline,
              ...
              onSaved: (value) => _editedProduct = Book(name: _editedProduct.name,author: _editedProduct.author, description: value!, unitPrice: _editedProduct.unitPrice, imageUrl: _editedProduct.imageUrl, id: _editedProduct.id),
            ...
            Row(children: [
              ...
              Expanded(child: TextFormField(decoration: InputDecoration(labelText: 'Image URL'), focusNode: _imageFocusNode,
                ...
                onSaved: (value) => _editedProduct = Book(name: _editedProduct.name,author: _editedProduct.author, description: _editedProduct.description, unitPrice: _editedProduct.unitPrice, imageUrl: value!, id: _editedProduct.id),
                validator: (value) {
                  if(value!.isEmpty) { return 'Please provide a value'; }
